...
export default class Dropdown extends Component {
  ...
  render() {
    const { show, options, buttonText, searchFilter } = this.state,
          { className = '', multiple, onBlur, disabled, hideOnEmpty, ...others } = this.props;

    if (!options || !options.length) {
      return !hideOnEmpty && <section className={`dropdown btn-group ${className}`} {...others}>
        <div className="dropdown-inner">
          <TextField className="dropdown-text" value={this.getDisplayText()} placeholder={buttonText} disabled />
          <Button className="dropdown-toggle dropdown-toggle-split" disabled />
        </div>
      </section>
    }
    ...
    return <section className={`dropdown btn-group ${className}`} {...others}>
      <div className="dropdown-inner">
        <TextField className="dropdown-text" value={this.getDisplayText()} placeholder={buttonText} readOnly disabled={disabled} />
        <Button className="dropdown-toggle dropdown-toggle-split" onClick={this.toggleDropdown} disabled={disabled} />
      </div>
      {show && <div className="dropdown-menu show">
        {multiple && <div className="dropdown-action">
          <Button className="btn-block" onClick={() => this.toggleSelectAll(options)}>{this.isAllSelected() ? 'Deselect All' : 'Select All'}</Button>
        </div>}
        <div className="dropdown-action">
          <TextField value={searchFilter || ''} onChange={this.setSearchFilter} placeholder="Search..." autoFocus />
        </div>
